...
class Manifest {
  ...
  async generatePdf(source, media) {
    if (source === 'imdb') {
      ...
      doc.pipe(fs.createWriteStream('movies.pdf'))
      const table = { headers: ['Cover', 'Col1', 'Col2', 'Col3', 'Col4'], rows: [] }
      for (const prop in media) {
        if (media[prop].title && media[prop].title.length) {
          console.log(media[prop])
          const pathObj = path.parse(media[prop].jsonPath)
          const imagePath = path.join(pathObj.dir, 'metadata', media[prop].coverImage)

          let row1 = trim(media[prop].title, 80) + `\n${media[prop].runtime}`
          let row2 = `${media[prop].imdbRating}/10\n${media[prop].type}\n${media[prop].parentalRating}\n${media[prop].releaseYear ? media[prop].releaseYear.substring(0,4) : 'N/A'}`
          let dude
          if (media[prop].director) dude = `DIR: ${media[prop].director}`
          else if (media[prop].creator) dude = `CREATOR: ${media[prop].creator}`
          let row3 = trim(`${dude}, STAR: ${media[prop].stars.length ? media[prop].stars[0] : 'N/A'}, GENRES: ${media[prop].genres.join(',')}`, maxChars)
      ...
      for (const prop in media) {
        ...
        log('debug', `${media[prop].title}: ${media[prop].jsonPath}`)
        if (media[prop].title && media[prop].title.length) {
          const pathObj = path.parse(media[prop].jsonPath)
          const imagePath = path.join(pathObj.dir, 'metadata', media[prop].coverImage)
          ...
          let row3 = trim(`${trim(media[prop].rating,7)}\n${!media[prop].genres ? 'N/A' : media[prop].genres.join(', ')}`, 80)
          let row4 = trim(media[prop].description, maxChars, true)
          const row = [
            {
              type: 'image',
