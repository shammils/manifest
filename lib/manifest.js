...
class ManifestOld {
  ...
  async handler(event) {
    const media = await this.scanDirs(event.dirs)
    await this.generateManifest(media)
    log('info', 'thing probably complete')
    return
  }
  async generateManifest(media) {
    const maxChars = 105;
    const doc = new PDFDocument({margin:10})
    doc.pipe(fs.createWriteStream('animes.pdf'))
    const table = { headers: ['Cover', 'Titles 1,2', 'Year, Ep ct, dur', 'rating, genre', 'description'], rows: [] }
    for (const prop in media) {
      // I got fucked up data due to MAL blocking my IP for scraping. FUCK THEM!
      if (media[prop].title && media[prop].title.length) {
    ...
    for (let i = 0; i < dirs.length; i++) {
      const p = new Promise((resolve, reject) => {
        ...
        .on('data', item => {
          ...
          if (thing.name === 'cover') {
            // we only want 'metadata/cover.<ext>', ignore anything else
            const arr = thing.dir.replace(dirs[i], '').split('/')
            if (arr[arr.length-1] === 'metadata')
              coverImages.push(item.path)
          }
        })
        .on('end', () => {
          log('info', `found ${Object.keys(result).length} json files, ${coverImages.length} images in dir '${dirs[i]}'`)
          // combine applicable cover images with json files
          for (let j = 0; j < coverImages.length; j++) {
            const pathObj = path.parse(coverImages[j])
            const arr = pathObj.dir.replace(dirs[i], '').split('/')
            if (result[arr[arr.length-2]]) {
              result[arr[arr.length-2]].coverImage = coverImages[j]
            } else {
              log('warn', `cover image not in path structure '<media name>/metadata/cover.<ext>'. image path: ${coverImages[j]}`)
            }
          }
          resolve(result)
        })
      ...
      for (let prop in result) {
        // pull json files, build final result with data
        try {
          const data = JSON.parse(Buffer.from(fs.readFileSync(result[prop].jsonPath)))
          media[prop] = {
            ...data,
            imagePath: result[prop].coverImage,
          }
        } catch(err) {
          log('error', `'${result[prop].jsonPath}' invalid json`)
        }
