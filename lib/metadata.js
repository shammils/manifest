...
const getWikiCellValue = ($, prop, td) => {
  ...
  for (let i = 0; i < td.children.length; i++) {
    if ((td.children[i].name === 'a' || td.children[i].name === 'i' || td.children[i].name === 'b') && td.children[i].children[0].data.length) {
  ...
  */
  ...
  if (td.children.length > 1 &&
    ...
    console.log(`!!! skipping all data after the first for prop "${prop}" !!!`)
    return td.children[0].data
  }
