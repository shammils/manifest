...
const getWikiCellValue = ($, prop, td) => {
  ...
  // handle makeshift lists of strings
  if (prop.toLowerCase() === 'created by') {
    let res = []
    $(td).children().each((i, c) => {
      if (c.name === 'a') res.push(c.children[0].data)
      // what a hack of a hack. iterating over the children does not hit text in
      // the root of the table cell: have to call next in order to get it
      const next = c.next
      if (next && next.type === 'text' && !res.includes(next.data)) res.push(next.data)
    })
    return res
  }
  ...
  // dangerous because we could be skipping values
  if (td.children.length > 1 &&
    td.children[0].type === 'text') {
    console.log(`=${prop}, skipping 1 and up =`)
    return td.children[0].data
  }
  console.log(`--- failed to get anything for ${prop}`)
  return
}
      ...
      $('table.infobox.vevent tr').each((i, tr) => {
        ...
        if (th.attribs.class === 'infobox-label') {
          ...
          if (th.children.length > 2 || prop === undefined) {
            throw `shits fucked, figure it out. header obj: ${th}`
          }
